...
test('conta RP única no cadastro', async () => {
  ...
  const first = await app.inject({ method: 'POST', url: '/api/auth/register', payload: { name: 'User One', characterName: 'Cidadao Um', discordId: 'discord-cad-1', characterPhone: '555-0201', screenshot: { mimeType: 'image/png', fileName: 'cadastro.png', data: 'data:image/png;base64,aW1hZ2VtLWRlLXRlc3Rl' }, password: '12345678' } });
  assert.equal(first.statusCode, 201, first.body);
  const second = await app.inject({ method: 'POST', url: '/api/auth/register', payload: { name: 'User Two', characterName: 'Cidadao Dois', discordId: 'discord-cad-2', characterPhone: '555-0202', screenshot: { mimeType: 'image/png', fileName: 'cadastro.png', data: 'data:image/png;base64,aW1hZ2VtLWRlLXRlc3Rl' }, password: '12345678' } });
